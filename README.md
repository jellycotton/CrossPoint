...
「Set Plate」を選択してオーバーレイモードに入ります。調整したいダイアログを完全に囲むようにプレート（青い線）を配置します。
...
「Move Cross」をクリックし、ボタンを配置したい場所にクロス（赤いマーク）を配置します。
...
青いプレートを調整してダイアログのサイズを変更します。次に、緑のサブプレートを追加して、ボタンの新しいサイズと位置を指定します。最後にパネルの「コピー」ボタンでコピーしたJSONをAIチャットに貼り付けて、リクエストを送信します。（例:「ダイアログのサイズはプレート、ボタンのサイズと位置をサブプレートの数値に合わせて変更して。」）
